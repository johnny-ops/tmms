...
export function OperatorApplicationsPage() {
  ...
  async function updateStatus(app: Application, status: AppStatus) {
    ...
    // When accepted: always create a driver_assignments record.
    // Vehicle is optional — operator can assign it later from the Assigned Drivers page.
    if (status === 'ACCEPTED') {
      const vehicleId = assignVehicleId[app.id] || null;

      // Check if an assignment already exists (avoid duplicates on re-approve)
      ...
      if (!existing) {
        ...
        // Already exists — just update the vehicle if one was selected
        await supabase.from('driver_assignments').update({ unit_id: vehicleId }).eq('id', existing.id);
      }

      // If a vehicle was selected, update application record too
      if (vehicleId) {
        await supabase.from('driver_applications').update({ unit_id: vehicleId }).eq('id', app.id);
      }
    }
