...
export function OperatorProfilePage() {
  ...
  // Filter by profile_id (auth user UUID) to get only this operator's own record
  const profileFilter = user?.id ? { column: 'profile_id', value: user.id } : undefined;
  const { data: operators, loading } = useTable<any>('operators', [], profileFilter ? { filter: profileFilter } : undefined);
  const operatorProfile = user?.id && operators.length > 0 ? operators[0] : null;
  ...
  async function handleDeleteAccount() {
    ...
    try {
      if (operatorProfile?.id) {
        await supabase.from('operators').delete().eq('id', operatorProfile.id);
      }
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div style={{ background: 'white', borderRadius: 8, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(to right, #3b82f6, #1e40af)', height: 120 }}></div>
        ...
        <div style={{ padding: '0 24px 24px 24px', position: 'relative' }}>
          ...
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0f172a', marginBottom: 4 }}>
            {operatorProfile?.full_name || user?.full_name || 'Operator Name'}
          </h2>
          <span className="badge badge-active" style={{ marginBottom: 24, display: 'inline-block' }}>
            VERIFIED OPERATOR
          </span>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.85rem', color: '#334155' }}>{operatorProfile?.contact_number || 'Not provided'}</span>
                </div>
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  <MapPin size={16} color="#64748b" style={{ marginTop: 2 }} />
                  <span style={{ fontSize: '0.85rem', color: '#334155' }}>{operatorProfile?.address || 'Not provided'}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 500 }}>OPERATOR LICENSE NUMBER</div>
                    <div style={{ fontSize: '0.85rem', color: '#0f172a', fontWeight: 600 }}>
                      {operatorProfile?.license_number || 'PENDING'}
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.85rem', color: '#0f172a', fontFamily: 'monospace' }}>
                      {operatorProfile?.id?.slice(0, 12) || user?.id.slice(0, 12)}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 32, maxWidth: 400, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: '#64748b', textAlign: 'center', marginBottom: 24, lineHeight: 1.6 }}>
              This will permanently delete your operator account and all associated data. This action <strong>cannot</strong> be undone.
            </p>
