...
export function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    console.log('[TMMS] Registration started for:', formData.email, 'role:', role);

    try {
      // STEP 1: Create the auth user
      console.log('[TMMS] Step 1: Creating auth user via signUp...');
      const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
        email: formData.email,
        password: formData.password,
        options: {
          data: { full_name: formData.full_name, role },
          // Do NOT pass emailRedirectTo — we handle flow manually
        },
      });

      console.log('[TMMS] signUp result:', { user: signUpData?.user?.id, error: signUpError?.message });
      ...
      // Even if email confirmation is ON, signUp returns a user object.
      // We sign in immediately with password to bypass the unconfirmed state.
      console.log('[TMMS] Step 2: Signing in with password...');
      const { data: signInData, error: signInError } = await supabase.auth.signInWithPassword({
        email: formData.email,
        password: formData.password,
      });

      console.log('[TMMS] signIn result:', { user: signInData?.user?.id, error: signInError?.message });

      // If email confirmation is required and sign-in fails with "Email not confirmed",
      // we still have the user ID from signUp — use it to insert the profile.
      ...
      // STEP 3: Insert driver/operator profile
      console.log('[TMMS] Step 3: Inserting', role, 'profile for user', userId);
      
      // Update the public.profiles record that was auto-created by Supabase trigger
      const { error: profileErr } = await supabase.from('profiles').update({
        role: role,
        full_name: formData.full_name
      }).eq('id', userId);
      
      if (profileErr) console.warn('[TMMS] Profile update warning:', profileErr.message);
      ...
      if (role === 'DRIVER') {
        ...
        else console.log('[TMMS] Driver profile created');
      } else {
        ...
        else console.log('[TMMS] Operator profile created');
      }

      // STEP 4: Navigate
      // If sign-in succeeded → go straight to dashboard
      // If email confirmation required → go to a simple "check your email" page
      if (signInData?.user) {
        console.log('[TMMS] Signed in! Redirecting to dashboard...');
        if (role === 'DRIVER') navigate('/driver/dashboard');
        else navigate('/operator/dashboard');
      } else {
        console.log('[TMMS] Email confirmation required. Going to verify page...');
        navigate(`/verify-otp?email=${encodeURIComponent(formData.email)}&role=${role}`);
      }

    } catch (err: any) {
      console.error('[TMMS] Registration failed:', err);
