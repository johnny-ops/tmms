...
import {
  Car, FileText, AlertTriangle, UserCheck, ShieldCheck,
...
export function OperatorDashboard() {
  ...
  const { user } = useAuth();
  const filter = user?.operator_id ? { column: 'operator_id', value: user.operator_id } : undefined;
  
  const { data: vehicles } = useTable<any>('vehicles', [], { filter });
  const { data: franchises } = useTable<any>('franchises', [], { filter });
  const { data: drivers } = useTable<any>('drivers', [], { filter });
  // For tickets, filtering by operator_id directly may require it in DB, otherwise might need complex join. 
  // Assuming RLS or simple filter works if schema has it. 
  const { data: tickets } = useTable<any>('traffic_tickets');
            ...
            {tickets.slice(0, 5).map(t => {
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td style={{ fontWeight: 500 }}>{t.violation_type_id}</td>
